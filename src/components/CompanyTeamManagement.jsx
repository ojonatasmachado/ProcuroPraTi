...
const CompanyTeamManagement = ({ company, access, onEnabled, onClose }) => {
  ...
  return <section className="mx-auto max-w-4xl space-y-5">
      ...
      {!access?.enabled ? <Card><CardHeader><CardTitle className="flex items-center gap-2"><ShieldCheck className="text-primary" /> Ativar controle de acessos</CardTitle><CardDescription>Você, como responsável, já está identificado pelo login da empresa e não precisa de PIN. Ative para poder cadastrar colaboradores, que entram com usuário e PIN próprios.</CardDescription></CardHeader><CardContent className="space-y-4">{error && <p role="alert" className="text-sm font-medium text-destructive">{error}</p>}<Button onClick={enable} disabled={saving}>{saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <KeyRound className="mr-2 h-4 w-4" />}Ativar controle de acessos</Button></CardContent></Card> : <>
