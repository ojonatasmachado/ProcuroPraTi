...
const CompanyTeamManagement = ({ company, access, onEnabled, onClose }) => {
  ...
  const enable = async () => {
    ...
    setSaving(true);
    try {
      await dataService.enableCompanyTeamAccess(getCompanyDeviceId(), getCompanyDeviceName());
      toast({ title: 'Controle de acessos ativado', description: 'Agora você já pode adicionar colaboradores.' });
      await onEnabled();
