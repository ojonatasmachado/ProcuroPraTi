...
const CompanyTeamManagement = ({ company, access, onEnabled, onClose }) => {
  ...
  const [form, setForm] = useState(() => access?.enabled ? null : emptyOperator());
  ...
  const enable = async (event) => {
    ...
    try {
      ...
      await onEnabled();
      setForm(current => current || emptyOperator());
    } catch (enableError) {
  ...
  const saveOperator = async (event) => {
    event.preventDefault();
    setError('');
    if (!access?.enabled) return setError('Ative primeiro o controle de acessos no cartão acima.');
  ...
  return <section className="mx-auto max-w-4xl space-y-5">
      ...
      {!access?.enabled ? <Card><CardHeader><CardTitle className="flex items-center gap-2"><ShieldCheck className="text-primary" /> 1. Proteja o acesso do responsável</CardTitle><CardDescription>Crie seu PIN privado. Você já pode preencher os dados do primeiro colaborador no cartão seguinte e, depois da ativação, enviar o acesso.</CardDescription></CardHeader><CardContent><form onSubmit={enable} className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label>PIN do responsável</Label><PinField value={ownerPin} onChange={setOwnerPin} /></div><div className="space-y-2"><Label>Confirmar PIN</Label><PinField value={ownerPinConfirmation} onChange={setOwnerPinConfirmation} /></div></div>{error && <p role="alert" className="text-sm font-medium text-destructive">{error}</p>}<Button disabled={saving}>{saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <KeyRound className="mr-2 h-4 w-4" />}Ativar controle de acessos</Button></form></CardContent></Card> : <>
