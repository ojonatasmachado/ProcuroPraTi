...
const UserRegistration = ({ onRegister, onSaveRegistrationProgress, onLogin, onCollaboratorLogin, onResendConfirmation, onRequestPasswordReset, onBackToLanding, accountExists, cnpjExists, allStatesAndCities, initialUserType, initialCollaboratorMode = false }) => {
  ...
  const confirmPasswordRef = useSecureFieldRef(showConfirmPassword);
  const collaboratorPinRef = useSecureFieldRef(showCollaboratorPin);
            ...
            {collaboratorMode ? (
              ...
              {isRegistration && (userType === 'company' || registrationStage === 'personal') && (
                <div>
                  ...
                  <div className="relative min-w-0 overflow-hidden rounded-[10px]">
                    <Input key={showConfirmPassword ? 'confirm-password-visible' : 'confirm-password-hidden'} ref={confirmPasswordRef} id="confirmPassword" name="confirmPassword" type={showConfirmPassword ? 'text' : 'password'} style={{ WebkitTextSecurity: showConfirmPassword ? 'none' : 'disc' }} autoComplete="new-password" placeholder="Confirme sua senha" required value={formData.confirmPassword} onChange={handleInputChange} aria-invalid={Boolean(fieldErrors.confirmPassword) || (formData.confirmPassword.length > 0 && !passwordsMatch)} className={`bg-popover border-border pr-12 text-sm h-11 ${fieldErrors.confirmPassword ? 'border-danger' : ''}`} />
